    ...
    with col2:
        ...
        with st.expander("Where do these values come from?"):
            st.markdown(
                ...
                Furthermore, El Capitan has the capability of carrying out approximately $2^{60}$ operations per second. So, since we have the total number of operations it would take, as well as El Capitan's operation rate, we can divide $\frac{\text{Total \# of Operations}}{\text{Operation Rate}}$ to find the approximate amount of time that it would take (in seconds) to break MLWE. Since both values are powers of base 2, this amounts to simply subtracting the exponents. For a large enough time estimate, this value can be converted to years by assuming around $2^{25}$ seconds in a year and subtracting the estimate accordingly.
                """
            )