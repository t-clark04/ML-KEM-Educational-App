...
with tab1:
    ...
    with col3:
        st.markdown("""
                    Now, it's time for Alice to uncover $K$. To understand how, it is important to note that if Alice can recover $\\mu$, then she can backtrack to recover the seed $m$ and therefore re-generate $K$. However, the only person who can uncover $\\mu$ is Alice, precisely because she held onto her original secret vector $s$ in the formula $t = As + e$. Thus, in decapsulation, Alice:
