    ...
    with col2:
        ...
        with st.expander("Where do these values come from?"):
            st.markdown(
                ...
                The estimate is 'rough' because it approximates the speed at which the BKZ can solve MLWE by assuming it to be equivalent to the speed at which the SVP oracle can solve the exact SVP problem on a sublattice of block size $\\beta$. The most effective classical algorithm for solving the exact SVP problem, called the BGDL Sieve, has an operational complexity of $2^{0.292\\beta}$, which tells us the approximate number of operations a classical computer would need to break a MLWE instance of block size $\\beta$.

                Furthermore, El Capitan has the capability of carrying out approximately $2^{60}$ operations per second. So, since we have the total number of operations it would take, as well as El Capitan's operation rate, we can divide $Total # of Operations/Operation Rate$ to find the approximate amount of time that it would take (in seconds) to break MLWE. Since both values are powers of base 2, this operation can be simplified by subtracting the exponents. For a large enough time estimate, this value can be converted to years by assuming around $2^{25}$ seconds in a year and subtracting the estimate accordingly.
                """
            )