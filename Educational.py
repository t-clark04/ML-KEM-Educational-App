...
KEYGEN_STEPS = [
    ...
    },
    {
        ...
        "description": "Using the variables Â, $\hat{s}$, and $\hat{e}$ generated in the previous steps, Alice now computes $\hat{t} = \hat{A} \cdot \hat{s} + \hat{e}$. This equation is the basis of the cryptosystem, based on the module learning with errors problem. Essentially, if one party only knows the values of $\hat{t}$ and $\hat{A}$ but not $\hat{e}$, it is very, very difficult to recover $\hat{s}$. Here's what the variable $\hat{t}$ looks like at the end of this step:",
        ...
        if current_phase == 'KeyGen':
            ...
            if current_step_index < len(KEYGEN_STEPS):
                ...
                if current_step_index == len(KEYGEN_STEPS) - 1:
                    ...
                    st.success("✅ **Key Generation Complete!** Alice now sends her public encapsulation key ($\mathbf{ek}$) over to Bob. Click the button below to continue.")
