...
with tab3:
    ...
    st.markdown(
        "Now, let's imagine that we constructed a lattice from the columns of $\\tilde{A}$ (our basis vectors). We'll denote the lattice as $\\mathcal{L}(\\tilde{A})$ and say that $\\mathcal{L}(\\tilde{A}) = \\{ \\tilde{A}z : z \\in \\mathbb{Z}^{kN}$."
    )